...
interface PageProps {
  params: Promise<{ categoria: string; ciudad: string; negocio: string }>
}
...
export default async function NegocioSlugPage({ params }: PageProps) {
  ...
  let business
  try {
    business = await negociosService.getByFullSlugs(categoria, ciudad, negocio)
  } catch {
    notFound()
  }
  ...
  const businessUrl = `${baseUrl}/${categoria}/${ciudad}/${negocio}`

  // Breadcrumb items - Updated to new URL structure
  const breadcrumbItems = [
    { label: 'Inicio', href: '/' },
    { label: business.categoria, href: `/${categoria}` },
    { label: business.ciudad, href: `/${categoria}/${ciudad}` },
  ...
  const breadcrumbStructuredData = [
    ...
    { name: business.ciudad, url: `${baseUrl}/${categoria}/${ciudad}` },
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <ContactButtons
                ...
                whatsappUrl={business.whatsapp ? formatWhatsAppUrl(business.whatsapp, `Hola, me interesa conocer más sobre ${business.nombre}`) : null}
                websiteUrl={business.sitio_web}
              />
